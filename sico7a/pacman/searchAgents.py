...
def foodHeuristic(state, problem):
    ...
    import sys                          # Just to get the maximum distance
    position, foodGrid = state          # Stores Pacman current 'position' and all food positions as a grid (foodGrid)
    foodList = foodGrid.asList()        # Changes food positions from grid to list of positions
    remFood = len(foodList)             # Stores the number of remaining food
    if remFood == 0:                    # If theres no food left
        return 0                            # Returns 0
    else:                               # Otherwise
